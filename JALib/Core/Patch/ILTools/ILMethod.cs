...
public class ILMethod : IEnumerator<ILCode>, ILTool {
    ...
    public ILMethod(IEnumerable<CodeInstruction> instructions, MethodInfo method) {
        ...
        foreach(CodeInstruction instruction in instructions) {
            ...
            if(instruction.opcode == OpCodes.Isinst) stack.Push(new IlIsInstance(Pop(stack), (Type) instruction.operand));
            if(instruction.opcode == OpCodes.Conv_R_Un) stack.Push(new ILConvert(Pop(stack), typeof(float)));
            if(instruction.opcode == OpCodes.Unbox) stack.Push(new ILUnbox(Pop(stack), (Type) instruction.operand));
            if(instruction.opcode == OpCodes.Throw) Codes.Add(new ILThrow(Pop(stack)));
            if(instruction.opcode == OpCodes.Ldfld) stack.Push(new ILFieldGet((FieldInfo) instruction.operand, Pop(stack)));
            if(instruction.opcode == OpCodes.Ldflda) stack.Push(new ILFieldPointerGet((FieldInfo) instruction.operand, Pop(stack)));
            if(instruction.opcode == OpCodes.Stfld) stack.Push(new ILFieldSet((FieldInfo) instruction.operand, Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldsfld) stack.Push(new ILFieldGet((FieldInfo) instruction.operand));
            if(instruction.opcode == OpCodes.Ldsflda) stack.Push(new ILFieldPointerGet((FieldInfo) instruction.operand));
            if(instruction.opcode == OpCodes.Stsfld) stack.Push(new ILFieldSet((FieldInfo) instruction.operand, Pop(stack)));
            if(instruction.opcode == OpCodes.Stobj) throw new NotSupportedException("Stobj is not supported");
            if(instruction.opcode == OpCodes.Conv_Ovf_I1_Un) stack.Push(new ILConvert(Pop(stack), typeof(sbyte)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I2_Un) stack.Push(new ILConvert(Pop(stack), typeof(short)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I4_Un) stack.Push(new ILConvert(Pop(stack), typeof(int)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I8_Un) stack.Push(new ILConvert(Pop(stack), typeof(long)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U1_Un) stack.Push(new ILConvert(Pop(stack), typeof(byte)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U2_Un) stack.Push(new ILConvert(Pop(stack), typeof(ushort)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U4_Un) stack.Push(new ILConvert(Pop(stack), typeof(uint)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U8_Un) stack.Push(new ILConvert(Pop(stack), typeof(ulong)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I_Un) stack.Push(new ILConvert(Pop(stack), typeof(IntPtr)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U_Un) stack.Push(new ILConvert(Pop(stack), typeof(UIntPtr)));
            if(instruction.opcode == OpCodes.Box) stack.Push(new ILBox(Pop(stack), (Type) instruction.operand));
            if(instruction.opcode == OpCodes.Newarr) stack.Push(new ILNewArr(Pop(stack), (Type) instruction.operand));
            if(instruction.opcode == OpCodes.Ldlen) stack.Push(new ILLength(Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelema) stack.Push(new ILArrayPointerGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_I1) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_U1) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_I2) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_U2) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_I4) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_U4) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_I8) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_I) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_R4) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_R8) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem_Ref) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_I) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_I1) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_I2) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_I4) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_I8) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_R4) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_R8) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem_Ref) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldelem) stack.Push(new ILArrayGet(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Stelem) stack.Push(new ILArraySet(Pop(stack), Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Unbox_Any) stack.Push(new ILUnbox(Pop(stack), (Type) instruction.operand));
            if(instruction.opcode == OpCodes.Conv_Ovf_I1) stack.Push(new ILConvert(Pop(stack), typeof(sbyte)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U1) stack.Push(new ILConvert(Pop(stack), typeof(byte)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I2) stack.Push(new ILConvert(Pop(stack), typeof(short)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U2) stack.Push(new ILConvert(Pop(stack), typeof(ushort)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I4) stack.Push(new ILConvert(Pop(stack), typeof(int)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U4) stack.Push(new ILConvert(Pop(stack), typeof(uint)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I8) stack.Push(new ILConvert(Pop(stack), typeof(long)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U8) stack.Push(new ILConvert(Pop(stack), typeof(ulong)));
            if(instruction.opcode == OpCodes.Refanyval) throw new NotSupportedException("Refanyval is not supported");
            if(instruction.opcode == OpCodes.Ckfinite) stack.Push(new ILCheckFinite(Pop(stack)));
            if(instruction.opcode == OpCodes.Mkrefany) throw new NotSupportedException("Mkrefany is not supported");
            if(instruction.opcode == OpCodes.Ldtoken) stack.Push(new ILToken((MemberInfo) instruction.operand));
            if(instruction.opcode == OpCodes.Conv_U2) stack.Push(new ILConvert(Pop(stack), typeof(ushort)));
            if(instruction.opcode == OpCodes.Conv_U1) stack.Push(new ILConvert(Pop(stack), typeof(byte)));
            if(instruction.opcode == OpCodes.Conv_I) stack.Push(new ILConvert(Pop(stack), typeof(IntPtr)));
            if(instruction.opcode == OpCodes.Conv_Ovf_I) stack.Push(new ILConvert(Pop(stack), typeof(IntPtr)));
            if(instruction.opcode == OpCodes.Conv_Ovf_U) stack.Push(new ILConvert(Pop(stack), typeof(UIntPtr)));
            if(instruction.opcode == OpCodes.Add_Ovf) stack.Push(new ILAdd(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Add_Ovf_Un) stack.Push(new ILAdd(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Mul_Ovf) stack.Push(new ILMul(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Mul_Ovf_Un) stack.Push(new ILMul(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Sub_Ovf) stack.Push(new ILSub(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Sub_Ovf_Un) stack.Push(new ILSub(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Endfinally) throw new NotSupportedException("Endfinally is not supported");
            if(instruction.opcode == OpCodes.Leave) throw new NotSupportedException("Leave is not supported");
            if(instruction.opcode == OpCodes.Leave_S) throw new NotSupportedException("Leave.s is not supported");
            if(instruction.opcode == OpCodes.Stind_I) throw new NotSupportedException("Stind.i is not supported");
            if(instruction.opcode == OpCodes.Conv_U) stack.Push(new ILConvert(Pop(stack), typeof(UIntPtr)));
            if(instruction.opcode == OpCodes.Arglist) throw new NotSupportedException("Arglist is not supported");
            if(instruction.opcode == OpCodes.Ceq) stack.Push(new ILEqual(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Cgt) stack.Push(new ILGreater(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Cgt_Un) stack.Push(new ILGreater(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Clt) stack.Push(new ILLess(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Clt_Un) stack.Push(new ILLess(Pop(stack), Pop(stack)));
            if(instruction.opcode == OpCodes.Ldftn) throw new NotSupportedException("Ldftn is not supported");
            if(instruction.opcode == OpCodes.Ldvirtftn) throw new NotSupportedException("Ldvirtftn is not supported");
            if(instruction.opcode == OpCodes.Ldarg) stack.Push(new ILParameterGet(Parameters[(int) instruction.operand]));
            if(instruction.opcode == OpCodes.Ldarga) stack.Push(new ILParameterPointerGet(Parameters[(int) instruction.operand]));
            if(instruction.opcode == OpCodes.Starg) stack.Push(new ILParameterSet(Parameters[(int) instruction.operand]));
            if(instruction.opcode == OpCodes.Ldloc) stack.Push(new ILLocalGet(GetLocal((LocalBuilder) instruction.operand)));
            if(instruction.opcode == OpCodes.Ldloca) stack.Push(new ILLocalPointerGet(GetLocal((LocalBuilder) instruction.operand)));
            if(instruction.opcode == OpCodes.Stloc) stack.Push(new ILLocalSet(GetLocal((LocalBuilder) instruction.operand), Pop(stack)));
            if(instruction.opcode == OpCodes.Localloc) throw new NotSupportedException("Localloc is not supported");
            if(instruction.opcode == OpCodes.Endfilter) throw new NotSupportedException("Endfilter is not supported");
            if(instruction.opcode == OpCodes.Unaligned) throw new NotSupportedException("Unaligned is not supported");
            if(instruction.opcode == OpCodes.Volatile) throw new NotSupportedException("Volatile is not supported");
            if(instruction.opcode == OpCodes.Tailcall) throw new NotSupportedException("Tailcall is not supported");
            if(instruction.opcode == OpCodes.Initobj) throw new NotSupportedException("Initobj is not supported");
            if(instruction.opcode == OpCodes.Constrained) throw new NotSupportedException("Constrained is not supported");
            if(instruction.opcode == OpCodes.Cpblk) throw new NotSupportedException("Cpblk is not supported");
            if(instruction.opcode == OpCodes.Initblk) throw new NotSupportedException("Initblk is not supported");
            if(instruction.opcode == OpCodes.Rethrow) Codes.Add(new ILRethrow());
            if(instruction.opcode == OpCodes.Sizeof) throw new NotSupportedException("Sizeof is not supported");
            if(instruction.opcode == OpCodes.Refanytype) throw new NotSupportedException("Refanytype is not supported");
            if(instruction.opcode == OpCodes.Readonly) throw new NotSupportedException("Readonly is not supported");
